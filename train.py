...
def main():
    ...
    print(
        "Train and test data splitted, train shape: ",
        train.shape,
        "test shape: ",
        test.shape,
    )
    ...
    labels_test_set = test_dataset.data["label"].values  # type: ignore
    test_loader = DataLoader(test_dataset, batch_size=1, shuffle=False)
    print("Test dataloader is ready")

    # sample a subset of train set
    print(
        "Sampling train data, with sampling percentage: ",
    ...
    for train_index, val_index in kfold.split(train_sampled):
        ...
        print(
            "Train and validation data splitted, train shape: ",
            train.shape,
            "validation shape: ",
            val.shape,
        )
        ...
        for ep in range(epochs):  # loop over the dataset multiple times
            ...
            print_training_statistics(
                ep, average_train_loss, average_val_loss, val_accuracy
            )
        ...
        accuracy, recall, precision, f1 = get_model_metrics(
            predictions_binarized, labels_test_set
        )
    ...
    averaged_predictions, averaged_predictions_binarized = model_averaged_predictions(
        prediction_matrix
    )
    assert len(averaged_predictions) == len(
        labels_test_set
    ), "Average predictions and labels have different length"
    ...
    if results_config["save_models"]:
        for i in range(n_splits):
            torch.save(
                models[i + 1].state_dict(),
                f"plots/{save_folder_name}/models/model_{i+1}.pth",
            )
    ...
    print(
        f"Number of non-bacilli in test set: {len(labels_test_set) - bacilli_in_test}"
    )
    ...
    for i in range(n_splits):
        ...
        fig = plt.gcf()
        fig.savefig(
            f"plots/{save_folder_name}/confusion_matrices/confusion_matrix_model_{i+1}.png"
        )
        # close figure
        plt.close(fig)
    ...
    print(
        "Plot of training and validation losses of different models saved in plots folder"
    )
    ...
    for i in range(n_splits):
        ...
        results.to_csv(
            f"plots/{save_folder_name}/results/results_model_{i+1}.csv", index=False
        )

    # save metrics_on_test dictionary as csv
    metrics_on_test_df = pd.DataFrame(metrics_on_test)
    metrics_on_test_df.to_csv(
        f"plots/{save_folder_name}/metrics_on_test.csv", index=False
    )

    cm = confusion_matrix(
        labels_test_set, averaged_predictions_binarized, normalize="true"
    )
    # plot confusion matrix using heatmap
    cm_analysis(
        labels_test_set,
        averaged_predictions_binarized,
        f"plots/{save_folder_name}/confusion_matrix.png",
        labels=[0, 1],
        classes=["Non-bacilli", "Bacilli"],
    )
    print(
        "Confusion matrix with predictions averaged from all models saved in plots folder"
    )
