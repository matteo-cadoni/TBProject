...
try:
    ...
    warnings.warn(
        "The package napari is not installed. Install napari to enable visualization features. Attention: installation of napari differs from machine to machine. Make sure to choose the right installation command for your machine."
    )
    _has_napari = False
else:
    _has_napari = True
...
def preprocess(preprocess_config, tile):
    ...
    if (
        preprocess_config["algorithm"] == "otsu"
        or preprocess_config["algorithm"] == "hard"
    ):
...
def smear_pipeline(config, smear, loader):
    ...
    for i, img in enumerate(smear):
        ...
        if isinstance(cropped_images, str):
            ...
            if (
                labelling_dataset_config["create_dataset"]
                and postprocessing_config["crop"]
            ):
                if stats.shape[0] > 1:
                    ...
                    for l in range(0, len(labels)):
                        d = {
                            "image": [cropped_images[l]],
                            "label": [labels[l]],
                            "stats": [stats[l]],
                            "predictions_ellipse": [predictions_ellipse[l]],
                        }
                        df2 = pd.DataFrame(d)
                    ...
                    if save_config["save"]:
                        ...
                        labelled_data_path = os.path.join(
                            "dataframe/",
                            loader.dataset_name + "_" + str(i) + "_marina" + ".pkl",
                        )
                    ...
                    if save_config["save_stats"]:
                        ...
                        stats_dataframe_path = os.path.join(
                            "labelled_data",
                            "stats_" + loader.dataset_name + "_" + str(i) + ".pkl",
                        )
            ...
            if inference_config["do_inference"]:
                ...
                if inference_config["prediction"] == "SVM":
                    ...
                    (
                        ...
                        _,
                        predictions_ellipse,
                    ) = inference.ellipse_brute_prediction()
...
def tile_pipeline(config, img, loader):
    ...
    if (
        labelling_dataset_config["create_dataset"]
        or save_config["save"]
        or inference_config["prediction"] == "CNN"
        or inference_config["prediction"] == "STATS"
    ):
        cropping_function = Cropping(img, stats)
        cropped_images = cropping_function.crop_and_pad()
    ######## END CROPPING ########
    ...
    if save_config["save_stats"]:
        ...
        stats_dataframe = pd.DataFrame(stats)
        stats_dataframe_path = os.path.join(
            "labelled_data", "stats_" + loader.dataset_name + ".pkl"
        )
        stats_dataframe.to_pickle(stats_dataframe_path)
        print("Stats saved in: " + stats_dataframe_path)
    ######## END SAVING ########
    ...
    if inference_config["do_inference"]:
        ...
        if inference_config["prediction"] == "SVM":
            ...
            answer = input(
                "Do you want to see the scatter plot of the CNN predictions on the geometric projection? (y/n) "
            )
            if answer == "y":
                plt.plot(
                    np.arange(0, np.max(coordinates[:, 0]), 0.1),
                    (1 / 1.5) * np.arange(0, np.max(coordinates[:, 0]), 0.1),
                    color="blue",
                )
                plt.plot(
                    np.arange(0, np.max(coordinates[:, 0]), 0.1),
            ...
            answer = input(
                "Do you want to see the scatter plot of the geometric projection? (y/n) "
            )
            if answer == "y":
        ...
        answer = input(
            "Do you want to visualize the inference results in napari? (y/n)"
        )
